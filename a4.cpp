...
public:
     ...
     * such as inserting, searching, and checking prefixes.
     *
     * @note The root node is initially empty and does not represent any character.
     * @post A new Trie is created with an empty root node.
     */
    Trie() {
        root = new TrieNode();
    }
     ...
     *
     * @details This method traverses the Trie character by character to determine 
     * if the complete word is present. It checks two conditions:
     * 1. All characters of the word must exist in the Trie's path
     * 2. The last node must be marked as the end of a word
     *
     * Time Complexity: O(m), where m is the length of the word
     * Space Complexity: O(1), as it uses a constant amount of extra space
     *
     * @note Returns false if any character is not found or the word is not 
     * explicitly inserted as a complete word.
     *
     * @example
     * Trie trie;
     * trie.insert("apple");
     ...
     * If all characters in the prefix can be found in the Trie's path, it returns true.
