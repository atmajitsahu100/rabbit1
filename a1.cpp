...
public:
     ...
     * @post A new Trie is created with an empty root node ready for word insertions.
     */
    Trie() {
        root = new TrieNode();
    }
     ...
     * The word is considered found only if all characters are present and the last node 
     * is marked as the end of a complete word.
     *
     * @note Time complexity: O(m), where m is the length of the word.
     ...
     * It returns true if all characters in the prefix are found in the Trie's path,
     * regardless of whether the path represents a complete word.
     *
     * @note Time complexity: O(m), where m is the length of the prefix.
     * @note Space complexity: O(1), as it only uses a single pointer for traversal.
     *
     * @example
     * Trie trie;
     * trie.insert("apple");
