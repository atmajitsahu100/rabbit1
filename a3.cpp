...
public:
     ...
     *
     * Initializes a TrieNode with no word ending status.
     * By default, the node is not marked as the end of a word.
     *
     * @note The node starts with an empty set of children and is not considered a complete word.
     */
    TrieNode() {
        isEndOfWord = false;
    }
...
public:
     ...
     *
     * @details This method adds a word to the Trie by creating nodes for each character
     * if they do not already exist. The last node of the word is marked as the end of a word.
     *
     * @param word The string to be inserted into the Trie.
     *
     * @note Time complexity: O(m), where m is the length of the word.
     * @note Space complexity: O(m) in the worst case, where no characters are shared.
     *
     * @throws None
     */
     ...
     * It checks each character's existence in the current node's children and ensures 
     * that the last node represents a complete word by checking the isEndOfWord flag.
     *
     ...
     * - The word is a prefix of another word but not a complete word itself
     *
     * @example
     * Trie trie;
     * trie.insert("apple");
     ...
     * @complexity Space complexity: O(1), as it only uses a constant amount of extra space.
     *
     * @example
     * Trie trie;
     * trie.insert("apple");
